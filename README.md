多模态设备运维 RAG Agent  
# 运维系统、知识问答和闲聊系统框架  单机运行standalone_chat.py
本项目是基于 LangGraph 构建的设备运维场景智能体系统，集成知识问答与闲聊双核心能力，为 LangGraph 多智能体开发的学习版实现。  
1.系统整体架构  
用户输入经意图分类后，分流至业务（查询、监控、自检、设置、提单）与非业务（知识问答、闲聊系统）处理：  
用户输入 → 意图分类（intent_cls）  
分支处理：  
业务意图 → 查询、监控、自检、设置、提单（仅demo代码）  
非业务意图 → 知识问答系统（query_agent）：基于 React Agent 调用工具完成业务查询  
            闲聊系统（chit_chat）：经安全过滤后调用 LLM 生成闲聊回复  
# 前端对接文档  
## 一、服务部署  
### 1. 环境依赖  
```bash
